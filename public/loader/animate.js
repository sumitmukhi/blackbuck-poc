window.onload = function () {//Change this Value to set the percentage
  ...
  function animate() {
    ...
    //main arc
    ctx.beginPath();
    ctx.strokeStyle = rotation >= 0.75 * Math.PI ? "#d71e48" : "#d71e48";
    ctx.lineWidth = "3";
    let radius = 174;
    ...
    const DrawDottedLine = (x1, y1, x2, y2, dotRadius, dotCount, dotColor) => {
      ...
      if (rotation >= 0.75 * Math.PI) {
        dotColor = degOfLine <= degOfNeedle ? "#d71e48" : "#d71e48";
      } else {
        dotColor = degOfLine <= degOfNeedle ? dotColor : "#d71e48";
      }
    ...
    for (let k = 0; k < firstDottedLineDots.length; k++) {
      ...
      DrawDottedLine(x, y, 175, 175, 1.75, 30, "#d71e48");
    }
    ...
    ctx.lineTo(x + 10, y + 10);
    ctx.closePath();
    ctx.fillStyle = rotation >= 0.75 * Math.PI ? "#d71e48" : "#d71e48";
    ctx.fill();
    ctx.restore();
